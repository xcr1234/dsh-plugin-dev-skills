...
| --- | --- | --- |
| [hello-plugin](./hello-plugin) | Plugin entry point (`name`/`apply`), lifecycle cleanup with `ctx.effect`, bundle manifest (`dsh.bundle`) and patch layer | `dsh plugin --profile demo add ./examples/hello-plugin` |
| [greet-tool](./greet-tool) | Registering a model-facing tool with `defineTool` (typed parameters, canonical output value, `output.render`) | `dsh plugin --profile demo add ./examples/greet-tool` |
